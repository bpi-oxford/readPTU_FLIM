...
def Process_Frame(im_sync,im_col,im_line,im_chan,im_tcspc,head,cnum = 1):
    ...
    tcspc_pix = np.zeros((nx, ny, Ngate, maxch_n*cnum)) # X-Y-Tau-CH*Pulse
    timeF = [None] * maxch_n*cnum
    tag = np.zeros((nx, ny, maxch_n, cnum)) #XYCP
    ...
    for ch in range(maxch_n):
        for p in range(cnum):
            ...
            timeF[ch*cnum + p] = np.round(im_sync[ind] / SyncRate / Resolution / 1e-9) + im_tcspc[idx].astype(np.int64)  # in tcspc bins 
